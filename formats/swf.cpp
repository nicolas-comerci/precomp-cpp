...
public:
  SwfPrecompressor(std::vector<unsigned char>&& _pre_deflate_header, const std::function<void()>& _progress_callback) :
    DeflateWithHeaderPrecompressor(std::move(_pre_deflate_header), _progress_callback) {}
...
std::unique_ptr<PrecompFormatPrecompressor> SwfFormatHandler::make_precompressor(Precomp& precomp_mgr, const std::span<unsigned char>& buffer) {
  return std::make_unique<SwfPrecompressor>(std::vector(buffer.data() + 3, buffer.data() + 10), [&precomp_mgr]() { precomp_mgr.call_progress_callback(); });
}
