...
public:
	explicit GZipFormatHandler(std::vector<SupportedFormats> _header_bytes, Tools* _precomp_tools, std::optional<unsigned int> _depth_limit = std::nullopt)
  ...
  void write_pre_recursion_data(RecursionContext& context, PrecompFormatHeaderData& precomp_hdr_data) override;
	...
	static GZipFormatHandler* create(Tools* tools) {
		return new GZipFormatHandler({ D_GZIP }, tools);
	}
};
