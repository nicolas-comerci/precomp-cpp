...
public:
	explicit GifFormatHandler(std::vector<SupportedFormats> _header_bytes, Tools* _precomp_tools, std::optional<unsigned int> _depth_limit = std::nullopt)
	...
	static GifFormatHandler* create(Tools* tools) {
		return new GifFormatHandler({ D_GIF }, tools);
	}
};
