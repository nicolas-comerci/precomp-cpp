...
public:
	explicit PdfFormatHandler(std::vector<SupportedFormats> _header_bytes, std::optional<unsigned int> _depth_limit = std::nullopt)
	...
	static PdfFormatHandler* create() {
		return new PdfFormatHandler({ D_PDF });
	}
};
